...
public class AdminHomeDao {
	...
	public List<MonthStatsVo> getMonthlySalesAndSignupStats(Connection conn) throws Exception {
		...
		String sql = "SELECT TO_CHAR(SL.ENROLL_DATE, 'YY/MM') AS MONTH, SUM(SL.SALES) AS TOTAL_SALES, COUNT(DISTINCT M.MEMBER_NO) AS SIGNUP_COUNT FROM SALES_LOG SL LEFT JOIN MEMBER M ON TO_CHAR(SL.ENROLL_DATE, 'YY/MM') = TO_CHAR(M.MEMBER_SIGN_DATE, 'YY/MM') GROUP BY TO_CHAR(SL.ENROLL_DATE, 'YY/MM') ORDER BY TO_CHAR(SL.ENROLL_DATE, 'YY/MM') DESC";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		ResultSet rs = pstmt.executeQuery();
